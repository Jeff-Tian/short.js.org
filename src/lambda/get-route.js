...
export function handler(event, context, callback) {
  ...
  // where is the data?
  var url = 'https://api.netlify.com/api/v1/forms/' +
      process.env.ROUTES_FORM_ID + '/submissions/?access_token=' +
    ...
    if (!err && response.statusCode === 200) {
      ...
      for (var item in routes) {
        ...
        if (routes[item].data.code === code) {
          console.log('We searched for ' + code + ' and we found ' +
              routes[item].data.destination);
          return callback(null, {
            statusCode: 200,
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(
                { code: code, url: routes[item].data.destination }),
          });
        }
      }
    } else {
